...
window.updateData = function(field, value, isSub = false, isCommon = false, shouldRender = true) {
  if (isSub) {
...
window.submitFinal = async function() {
  ...
  // 구글 스프레드시트 앱스 스크립트가 파라미터를 최상단(Root)에서 찾을 수 있도록 평탄화(Flatten)
  const payload = {
    action: "final",
    timestamp: new Date().toISOString(),
    name: formData.name,
...
function renderStep() {
      ...
      <div class="cf-space-y-6">
        ...
        <div class="cf-space-y-4">
          <div>
            ...
            <input type="text" class="cf-input" placeholder="홍길동" value="${formData.name}" onchange="updateData('name', this.value, false, false, false)">
          </div>
          <div class="cf-flex-gap-4">
            <div style="flex:1;">
              <label class="cf-label">연령대</label>
              <select class="cf-input" onchange="updateData('ageGroup', this.value, false, false, false)" style="cursor: pointer; appearance: none; background: transparent url('data:image/svg+xml;utf8,<svg xmlns=\\'http://www.w3.org/2000/svg\\' width=\\'24\\' height=\\'24\\' viewBox=\\'0 0 24 24\\' fill=\\'none\\' stroke=\\'currentColor\\' stroke-width=\\'2\\' stroke-linecap=\\'round\\' stroke-linejoin=\\'round\\'><polyline points=\\'6 9 12 15 18 9\\'></polyline></svg>') no-repeat right 4px center; padding-right: 24px;">
            ...
            <div style="flex:1;">
              ...
              <div class="cf-flex-gap-4">
                <label class="cf-radio-btn ${formData.gender === '남성' ? 'active' : ''}" style="padding: 8px 0;">
                  <input type="radio" class="cf-hidden" name="gender" value="남성" ${formData.gender === '남성' ? 'checked' : ''} onchange="updateData('gender', '남성', false, false, true)">남성
                </label>
                <label class="cf-radio-btn ${formData.gender === '여성' ? 'active' : ''}" style="padding: 8px 0;">
                  <input type="radio" class="cf-hidden" name="gender" value="여성" ${formData.gender === '여성' ? 'checked' : ''} onchange="updateData('gender', '여성', false, false, true)">여성
                </label>
              </div>
            </div>
          </div>
          <div>
            ...
            <input type="tel" class="cf-input" placeholder="010-0000-0000" value="${formData.phone}" onchange="updateData('phone', this.value, false, false, false)">
          </div>
          <div class="cf-pt-4 cf-space-y-2">
            <label class="cf-checkbox-label">
              <input type="checkbox" ${formData.privacyConsent ? 'checked' : ''} onchange="updateData('privacyConsent', this.checked, false, false, false)">
              <span>[필수] 개인정보 수집 및 이용 동의</span>
            </label>
            <label class="cf-checkbox-label">
              <input type="checkbox" ${formData.marketingConsent ? 'checked' : ''} onchange="updateData('marketingConsent', this.checked, false, false, false)">
              <span>[선택] 더웰샘 VIP 초청 이벤트 및 소식지 수신 동의</span>
            </label>
            <label class="cf-checkbox-label">
              <input type="checkbox" ${formData.thirdPartyConsent ? 'checked' : ''} onchange="updateData('thirdPartyConsent', this.checked, false, false, false)">
    ...
    if (qData.type === 'textarea') {
      contentHtml = `
        <div class="cf-space-y-8">
          ...
          <div class="cf-space-y-4">
            <textarea class="cf-textarea" onchange="updateData('${qKey}', this.value, true, false, false)" placeholder="여기에 자유롭게 작성해 주세요.">${formData.subAnswers[qKey]}</textarea>
      ...
      const customHtml = qData.custom ? `
        <div class="cf-flex-gap-2 mt-2">
          <input type="text" placeholder="기타 (직접 입력)" value="${isCustomVal ? formData.subAnswers[qKey] : ''}" onchange="updateData('${qKey}', this.value, true, false, false)" class="cf-input-flex">
    ...
    contentHtml = `
      <div class="cf-space-y-8">
        ...
        <div class="cf-space-y-3">
          ${btns}
          <div class="cf-flex-gap-2 mt-2">
            <input type="text" placeholder="기타 (직접 입력)" value="${isCustom ? formData.commonAnswers.medication : ''}" onchange="updateData('medication', this.value, false, true, false)" class="cf-input-flex">
        ...
        <div class="cf-space-y-3">
          ...
          <div class="cf-col-gap-3 pt-2">
            ...
            <div class="cf-flex-gap-2">
              <input type="text" placeholder="예: 류마티스 관절염" value="${formData.commonAnswers.disease !== '없음' ? formData.commonAnswers.disease : ''}" onchange="updateData('disease', this.value, false, true, false)" class="cf-input-flex">
    ...
    if (analysisLoading) {
      ...
      contentHtml = `
        <div class="cf-space-y-8 cf-animate-up">
          ...
          <div class="cf-space-y-6">
            <div>
              <label class="cf-label-bold">희망 상담 일정</label>
              <input type="datetime-local" value="${formData.q11Date}" onchange="updateData('q11Date', this.value, false, false, false)" class="cf-input-full">
            </div>
            <div>
              <label class="cf-label-bold">의료진에게 미리 전달하실 특이사항 (선택)</label>
              <textarea class="cf-textarea-small" onchange="updateData('q12Memo', this.value, false, false, false)" placeholder="이전에 받으신 시술이나 특별히 피하고 싶으신 치료법이 있다면 적어주세요.">${formData.q12Memo}</textarea>
            </div>
  ...
  const isNewStep = (lastRenderedStep !== step);
  const animateClass = isNewStep ? 'cf-animate-up' : '';
  ...
  modal.innerHTML = `
    ...
    <div class="cf-content-area ${animateClass}" id="cf-content">
      <div class="cf-w-full">
        ${contentHtml}
      </div>
    </div>
